...
services="addok addok-wrapper apache2 atd bind9 ceph clamav-daemon couchdb cron dibbler-client dovecot exim4 fail2ban fcgiwrap glassfish gitlab lm-sensors mailgraph memcached mongodb nagios-nrpe-server nginx npcd ntp openvpn pgbouncer php5-fpm postfix pure-ftpd-mysql rabbitmq-server redis-server resolvconf rmilter rspamd rsyslog shinken shorewall shorewall6 slapd spamassassin ssh uwsgi"
