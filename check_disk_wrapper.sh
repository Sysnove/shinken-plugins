...
$exe /usr/lib/nagios/plugins/check_disk -A -I '^/sys' -I '^/run' -I '^/dev$' -I tmpfs -I borgfs -X tmpfs -X fuse.sshfs -X fuse.nvim -X devtmpfs -X aufs -X overlay -X overlay2 $(findmnt --raw | grep '\[' | awk '{print "-I "$1}' | xargs) $@
exit $?
