...
$exe $@ -I '^/sys' -I '^/run' -I '^/dev$' -I borgfs -I tmpfs -X borgfs -X tmpfs -X fuse.sshfs -X fuse.FlexNetFs -X cifs -X fuse.nvim -X fuse.glusterfs -X devtmpfs -X aufs -X overlay -X overlay2 $(findmnt --raw | grep '\[' | awk '{print "-I "$1}' | xargs)
