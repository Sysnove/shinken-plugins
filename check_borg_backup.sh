...
if [ -n "$WARN_RATIO" ] && [ $size_ratio -lt "$WARN_RATIO" ] && [ "$last_original_size_gb" -gt 5 ]; then
    echo "WARNING: ${last_original_size_gb}GB backed up today, but ${unique_size_gb}GB unique chunks in repo. Please check server activity and backup excludes. $stats_msg"
    exit 1
fi
