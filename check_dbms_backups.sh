    ...
    if $running || $datadir; then
        if [ -f "${dbms_backupdirs[$dbms]}/README" ]; then
            ...
            if [ "$dbms" == "elasticsearch" ]; then
                if ! out=$(/usr/local/nagios/plugins/check_all_files_age.sh "${dbms_backupdirs[$dbms]}" "-maxdepth 2 -not -name incompatible-snapshots -type f"); then
                    echo "$dbms: $out"
                    exit 2
                fi
            elif [ "$dbms" == "couchbase" ]; then
                # On couchbase we have weekly compacting so files can be one week old
                if ! out=$(/usr/local/nagios/plugins/check_all_files_age.sh "${dbms_backupdirs[$dbms]}"  "-mindepth 1 -maxdepth 3 -type d -not -name logs -not -name stats -not -name lost+found" 10140 20280); then
                    echo "$dbms: $out"
                    exit 2
                fi
            else
