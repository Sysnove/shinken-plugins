...
#
# CopyLeft Guillaume Subiron <maethor@subiron.org>
# Based on check_smart.pl, by Roy Sigurd Karlsbakk <roy@karlsbakk.net>
#
...
if ($text_mode) {
	...
	while (<SMARTCTL>) {
        ...
        } elsif ($_ =~ /SAVE ALL DATA/ || $_ =~ /FAILED/) {
            print $_ if ($verbose);
            end(2);
        }
	...
	end(0) if ($out =~ /PASSED/ || $out =~ /OK/);
	end(2) if ($out =~ /SAVE ALL DATA/ || $out =~ /FAILED/);
	end(3);
} elsif ($exitcode_mode) {
