...
" > "$LAST_RUN_FILE"
    ...
    hitrate=$((((cache_hit - old_cache_hit) * 100) / (total_hit - old_total_hit)))
    reqpersec=$(bc <<< "scale=1; ($total_hit - $old_total_hit) / $period")

    echo "Varnish OK : $((total_hit - old_total_hit)) requests in ${period} seconds (cache hitrate ${hitrate}%) | ReqPerSec=$reqpersec CacheHitrate=${hitrate}%"
else
