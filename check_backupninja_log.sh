...
resume=$(echo "$last_line" | grep -Eo "[0-9]+ actions run.*")

if [ $((now_ts - last_line_ts)) -gt $((60*60*52)) ]; then
    echo "CRITICAL - Last backup has finished more than 52 hours from now ($last_line_date: $resume)"
    exit 2
fi
...
if [ "$nb_fatal" -gt 0 ]; then
    echo "CRITICAL - $nb_fatal fatal errors in last backupninja run ($last_line_date: $resume)"
    exit 2
fi

if [ "$nb_error" -gt 0 ]; then
    echo "CRITICAL - $nb_error errors in last backupninja run ($last_line_date: $resume)"
    exit 2
fi

if [ $((now_ts - last_line_ts)) -gt $((60*60*27)) ]; then
    echo "WARNING - Last backup has finished more than 27 hours from now ($last_line_date: $resume)"
    exit 1
fi

if [ "$nb_warning" -gt 0 ]; then
    echo "WARNING - $nb_warning warnings in last backupninja run ($last_line_date: $resume)"
    exit 1
fi

echo "OK - $last_line_date: $resume"
exit 0
